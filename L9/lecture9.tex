...
\begin{document}
 ...
 \begin{frame}{Exp3 Algorithm}
  \begin{algorithm*}[H]
    Set $u \gets (1/N, \dots, 1/N)$\;
    Set weights $w_i^1 \gets 1$ for all experts $i$\;
    \For{$t = 1$ to $T$}{
     Let $W^t = \sum_{i=1}^N w_i^t$\;
     Calculate $p_i^t \gets w_i^t/W^t$ for all $i$\;
     Calculate probabilities: $q^t = (1-\gamma) p^t + \gamma u$\;
     Choose expert $i_t$ randomly according to $q^t$\;
     Observe expert $i_t$'s loss $\ell_{i_t}^t$\;
     Set other experts losses: $\ell_i^t \gets 0$ for all $i \ne i_t$\;
     Calculate scaled loesss: $\hat{\ell}_i^t \gets \ell_i^t/q_i^t$ for all $i$\;
     Update weights: $w_i^{t+1} \gets w_i^t \cdot \exp(-\epsilon \hat{\ell}_i^t)$ for all $i$\;
    }
  \end{algorithm*}
 \end{frame}
   ...
   \begin{frame}{Exp3 Algorithm}
   \begin{center}
    \begin{tikzpicture}[x=1.75cm,y=1cm,font=\footnotesize]
     \draw[rounded corners, line width=1.5pt] (0, 0) rectangle (3, 4);
     \draw[line width=1pt, fill=lightblue] (0.5, 0.5) rectangle (1, 3.5);
     \draw[line width=1pt, fill=lightgreen] (2, 0.5) rectangle (2.5, 3.5) node[pos=.5] {$MW$};

     \draw[-stealth, black, thick] (-1,1.75) -- node[below] {$\ell^t$} (0.5, 1.75);
     \draw[stealth-, black, thick] (-1,2.25) -- node[above] {$q^t$} (0.5 , 2.25);

     \draw[-stealth, black, thick] (1,1.75) -- node[below] {$\hat{\ell}^t$} (2, 1.75);
     \draw[stealth-, black, thick] (1,2.25) -- node[above] {$p^t$} (2, 2.25);
 ...
 \begin{frame}{Analysis of Exp3}
  \begin{itemize}[<+->]
   \item $\E[\hat{\ell}_i^t] = \ell_i^t / q_i^t \cdot q_i^t + 0 \cdot (1 - q_i^t) = \ell_i^t$
   \item True loss is in $[0, 1]$, but $MW$ sees losses in $[0, N/\gamma]$
   \item From analysis of MW, we have:
   \[\frac{1}{T}~\E_{p}[\hat{L}_{MW}^T] \leq \frac{1}{T}~\hat{L}_k^T + \frac{N}{\gamma}\left(\epsilon + \frac{\ln(N)}{\epsilon\cdot T}\right)\]
   \item Taking expectation over both sides, we have:
   \[\frac{1}{T}~\E_{p}[L_{MW}^T] \leq \frac{1}{T}~L_k^T + \frac{N}{\gamma}\left(\epsilon + \frac{\ln(N)}{\epsilon\cdot T}\right)\]
  \end{itemize}
 \end{frame}
